      ...
      <div key={animationKey} className="w-full flex-1 min-h-0 flex flex-col items-center justify-center my-1 sm:my-2 z-20 overflow-hidden pointer-events-none">
        <div className="relative grid gap-x-1 sm:gap-x-2 md:gap-x-4 max-w-full px-2 mx-auto w-fit place-items-center" style={{ gridTemplateRows: 'auto minmax(10px, 6vh) auto', gridTemplateColumns: `repeat(${maxCount}, minmax(0, 1fr))` }}>
          ...
          {Array.from({ length: topCount }).map((_, i) => (
            <div key={`top-${i}`} className="row-start-1 flex justify-center items-center p-1 sm:p-2 md:p-3 z-10 w-full h-full" style={{ gridColumnStart: i + 1 }}>
          ...
          {showAnswer && Array.from({ length: minCount }).map((_, i) => (
            <div key={`line-${i}`} className="row-start-2 flex justify-center items-center z-20 w-full h-full" style={{ gridColumnStart: i + 1 }}>
              <div className="w-1 sm:w-1.5 md:w-2 h-full bg-pink-500 animate-draw-line origin-top" style={{ animationDelay: `${i * 0.1}s` }} />
            </div>
          ))}

          {Array.from({ length: bottomCount }).map((_, i) => (
            <div key={`bottom-${i}`} className="row-start-3 flex justify-center items-center p-1 sm:p-2 md:p-3 z-10 w-full h-full" style={{ gridColumnStart: i + 1 }}>
              <div className={`animate-pop-in ${showAnswer && !isTopGreater && i >= minCount ? 'animate-bounce-happy' : ''}`} style={{ animationDelay: `${i * 0.05}s` }}>
